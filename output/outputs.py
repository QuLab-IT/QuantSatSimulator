...
def get_data_header(protocol):
    ...
    if protocol.lower() == list_protocols[0].lower():
        ...
        header = "dt (s),ls (dB),QBERI,Pec,maxElev (deg),SKL (b),QBER,phiX," +\
                  "nX,nZ,mX,lambdaEC,sX0,sX1,vZ1,sZ1,mean photon no.,PxA," +\
                  "PxB,P1,P2,mu1,mu2,mu3,eps_c,eps_s,Pap,NoPass," +\
                  "fs (Hz),minElev (deg),shiftElev (deg),SysLoss (dB)"
        return header
    else:
...
def write_data(out_params,tOptimise,header,opt_head,outfile,fulldata,
               multidata,optdata):
    """
    ...
    if out_params['tFullData']:
        ...
        writeDataCSV(fulldata,out_params['out_path'],outfile,
...
def out_arrays(header,opt_head,tOptimise,tMultiOpt):
    """
    ...
    fulldata = np.empty(len(header.split(",")))
    
    #if tMultiOpt and (ni[0]*ni[1]*ni[2]*ni[3]) > 1:
    if tMultiOpt:
        multidata = np.empty(len(header.split(",")))
    else:
        multidata = None
    
    if tOptimise:
        # Initialise a data storage array for optimiser metrics
        optdata = np.empty(len(opt_head.split(",")))
    else:
        optdata = None
