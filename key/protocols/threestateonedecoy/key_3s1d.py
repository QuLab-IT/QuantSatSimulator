...
from .init_3s1d import *
from .func_3s1d import *
...
def key_length(x, args):
    ...
    P  = np.array([x[1],max(1-x[1],num_min)])
    ...
    if boundFunc in ['Chernoff','chernoff']:
        ...
        mXmin, mXplus = nXZpm(mu,P,mx_mu,eps_s)
        mXmax = np.sum(mXplus)
        ...
        # Returns the (3,) arrays mXmin and mXplus
        mXmin, mXplus = nXZpm_HB(mu,P,mx_mu,eps_s)
        mXmax = np.sum(mXplus) 
        # Returns the (3,) arrays mZmin and mZplus
        ...
        # Returns the (3,) arrays mXmin and mXplus
        mXmin, mXplus = nXZpm_inf(mu,P,mx_mu)
        mXmax = np.sum(mXplus) 
        # Returns the (3,) arrays mZmin and mZplus
    ...
    QBERx = Qber(PAx,PBx,nz,mX,nXZ,nZX)
    ...
    if boundFunc in ['Asymptotic','asymptotic']:
        ...
        l = max((sx0 + sx1 * (1 - h(phi_x)) - lambdaEC -
             6*np.math.log(19.0 / eps_s, 2) - np.math.log(2.0 / eps_c, 2)) * 
             heaviside(mu[0] - mu[1]), 0.0)
...
def key_length_sim(x,args):
    ...
    #print(args)
    # Safety check that the parameters satisfy the constraints
    C = bool_constraints(x[0],x[1],x[2],x[3],args['mu3'])
    if (not np.all(C)):
        return num_max  # ~1/0
